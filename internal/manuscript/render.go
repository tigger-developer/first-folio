...
// substitutePlaceholders resolves metadata, page, and semantic heading placeholders in a
// header/footer format string. Recognized tokens are replaced with escaped metadata or the
...
func naiveSubstitute(format string, meta Metadata) string {
	...
	for _, m := range placeholderRE.FindAllStringSubmatchIndex(format, -1) {
		...
		case "total-pages":
			out.WriteString("#folio-total-pages()")
